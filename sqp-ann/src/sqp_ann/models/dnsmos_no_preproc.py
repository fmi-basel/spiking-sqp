...
class DNSMOSNoPreproc(nn.Module):
    def __init__(self, n_fft, n_mels, channels, hidden_1, hidden_2, dropout, conv_activation=nn.ReLU(), conv_activation_param=None, gap=False, weight_quant=None):
        super(DNSMOSNoPreproc, self).__init__()
        self.n_fft = n_fft
        self.n_mels = n_mels
    ...
    def process_data(self, logmel_input):
        # preprocess and run forward pass
        mos_pred = self.forward(logmel_input).squeeze(-1)
        return mos_pred, logmel_input
