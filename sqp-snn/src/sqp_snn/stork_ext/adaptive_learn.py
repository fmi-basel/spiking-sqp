...
from .lif import CustomLIFGroup


class CustomAdaptLearnLIFGroup(CustomLIFGroup):
    def __init__(self, shape, tau_ada=1.0, **kwargs):
    ...
    def forward(self):
        ...
        new_mem = (self.dcy_mem * self.mem +
                   self.scl_mem * (self.syn - self.adapt_b * self.ada)) * (1.0 - rst)

        # decay of adaptation variable
        new_ada = self.dcy_ada * self.ada + self.scl_ada * (self.out)
